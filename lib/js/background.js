...
(function($, _, can, chrome, document, window){
    ...
    chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
        var stashURL = kmj.getLocalStore("URL"),
            ...
            insertCSS = true;// kmj.getLocalStore("InsertCSS", false, "boolean");

        if (insertCSS && changeInfo.status && changeInfo.status === "complete" && tab.url && tab.url.startsWith(stashURL) ){
            // console.log("ok including scripts", stashCSS);
            chrome.tabs.executeScript(tabId, {file: "lib/js/vendor/zepto.js", runAt: "document_end"});
            chrome.tabs.executeScript(tabId, {file: "lib/js/content_script.js", runAt: "document_end"});
            // chrome.tabs.insertCSS(tabId, {file: "lib/css/content_css.css"});
