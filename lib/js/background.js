    ...
    */
    ...
    var DEFAULT_VALUES = window.DEFAULT_VALUES,
        ...
        kmj = {
            ...
            opendNotifications: [],
            snoozedItems: [],
            addSnoozed: function(id){
                kmj.snoozedItems.push(parseInt(id));
                kmj.init();
            },
            authorItems:[],
            popupItems:[],
            ...
            handleResponse:function(data, listToPopulate){
                ...
                if (listToPopulate === "items"){
                    var snoozed = _.filter(kmj.json.values, function(obj){ return kmj.snoozedItems.indexOf(obj.id) !== -1}).length;
                    console.log("items update, ", kmj.json.size, unapproved, snoozed);
                    kmj.updateBrowserActionStatus( kmj.json.size - snoozed , unapproved - snoozed );
