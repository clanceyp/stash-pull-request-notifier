...
(function($, _, can, chrome, document, window){
    ...
    var console = window.console,
        ...
        kmj = {
            ...
            /**
             * Method, performs some actions on a list to add and update some list item peramiters
             * @id updateStatus
             * @memberOf kmj
             * @return void
             */
            updateStatus: function(){
                var active = _.filter(kmj.items, function(obj){ return !obj.snoozed }).length,
                    reviewerUnapproved = _.filter(kmj.items, function(obj){ return !obj.snoozed && obj.reviewerUnApproved }).length;
            ...
            processList: function(data, listToPopulate){
                var popupReverseOrder = kmj.getLocalStore("PopupReverseOrder", false, "boolean"),
                    other = {maxReviewersCount: 0};

                kmj[listToPopulate] = _.sortBy(data, function(item){
                ...
                kmj[listToPopulate].forEach(function(item){
                    ...
                    for (; i< item.reviewers.length; i++){
                        ...
                        statuses.push({
                            status: item.reviewers[i].status,
                            order: item.reviewers[i].status
                                .replace('NEEDS_WORK', '0')
                                .replace('APPROVED', '1'),
                            name: item.reviewers[i].user.displayName
                        });
                    }
                    item.authorReviewerStatus = 'NOT_FOUND';
                    for (var i in item.reviewers) {
                        if (item.reviewers[i].user.name === kmj.getLocalStore("Login")) {
                            item.authorReviewerStatus = item.reviewers[i].status;
                            break;
                        }
                    }
                    item.reviewerUnApproved = item.authorReviewerStatus === 'UNAPPROVED';
                    item.reviewerApproved = item.reviewer_approved = approved;
                    item.approveCount = approveCount;
                    item.statuses = statuses.sort(function (a, b) {
                        if (a.order < b.order) return -1;
                        if (a.order > b.order) return 1;
                        return 0;
                    });
                    item.other = other;
                    if (item.reviewers.length > other.maxReviewersCount) other.maxReviewersCount = item.reviewers.length;
                });
                kmj[listToPopulate].forEach(function (item) {
                    if (other.maxReviewersCount > item.statuses.length) {
                        for (var i = 0; i < other.maxReviewersCount - item.statuses.length; i++)
                            item.statuses.push({
                                status: 'EMPTY'
                            });
                    }
                });
                kmj[listToPopulate].sort(function (a, b) {
                    return b.reviewerUnApproved - a.reviewerUnApproved
                });
