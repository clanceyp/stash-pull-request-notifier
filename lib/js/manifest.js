	...
	OPTIONS = {
		FORMS : { //  new elements will show in options page, but for default_values the extension needs to be reloaded
			"options" : [
                ...
                {"name":"Login", "label": "Your login", "type": "text"},
		...
		DEFAULT_VALUES : {
            ...
            ,"PopupTemplate":'{{#stash}}'+
            '\n<details class="item{{#onlyAuthors}} item--reviewer-approved-{{ reviewerApproved }}{{/onlyAuthors}} item--reviewer-approved-count-{{approveCount}}" data-id="{{id}}">'+
            '\n	 <summary class="item__user-status--{{authorReviewerStatus}}">' +
            '\n     {{^onlyAuthors}}' +
            '\n         <i class="item__reviewers-reviewer_no-margin item__reviewers-reviewer--approved-{{authorReviewerStatus}}" title="Your vote is {{authorReviewerStatus}}"></i>' +
            '\n     {{/onlyAuthors}}' +
            '\n		{{#statuses}}<i class="item__reviewers-reviewer_statuses item__reviewers-reviewer--approved-{{status}}" title="{{name}}"></i>{{/statuses}}' +
            '\n     {{date createdDate}} {{title}}' +
            '\n  </summary>'+
            ...
            '\n			<h3>Reviewers:</h3> '+
            '\n			{{#reviewers}}'+
            '\n				<i class="item__reviewers-reviewer item__reviewers-reviewer--approved-{{status}}">{{user.displayName}}</i>'+
            '\n			{{/reviewers}}'+
            '\n		</div>'+
            '\n		<div class="cell">'+
            '\n			<h3>Participants:</h3> '+
            ...
            '\n            {{/reviewers}}'+
            '\n            {{#reviewers}}'+
            '\n                <i class="item__reviewers-reviewer item__reviewers-reviewer--approved-{{status}}">{{display_name}}</i>'+
            '\n            {{/reviewers}}'+
            '\n         </div>'+
            '\n         <div class="cell">'+
            '\n            <h3>Participants:</h3>'+
        ...
        HELP : {
            ...
            ,"DifferentiateApproved": 'Visually differentiate items that have been approved by you'
            ,"URL":'Your URL e.g. http://bitbucket.my-domain.com/'
            ,"Login":'Your site login e.g. UserName'
