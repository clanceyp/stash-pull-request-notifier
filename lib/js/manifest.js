	...
	OPTIONS = {
		FORMS : { //  new elements will show in options page, but for default_values the extension needs to be reloaded
			"options" : [
                ...
                {"name":"PopupTemplate","label":"Mustache template","type":"textarea",  parent:'advanced', className:"code allowDblClickReset"},
                ...
                {"name":"PopupCSS","label":"Additional CSS","type":"textarea",  parent:'advanced', className:"code"},
                {"name":"PopupDateformat","label":"Date format","type":"text", parent: 'advanced', className: "short"},
                {"name":"PopupIgnoreList","label":"Ignore list","type":"text", parent: 'advanced'},
                {"name":"PopupReverseOrder","label":"Reverse order","type":"checkbox"},
                {"name":"DifferentiateApproved","label":"Ghost approved PRs","type":"checkbox", parent: 'advanced'},
		...
		DEFAULT_VALUES : {
            ...
            ,"PopupTemplate": `{{#stash}}
    <details class="item item--reviewer-approved-{{ reviewerApproved }} item--reviewer-approved-count-{{approveCount}} {{#snoozed}}item--snoozed{{/snoozed}}" data-id="{{id}}">
    	 <summary title="Comment count, {{properties.commentCount}}. Merge outcome, {{properties.mergeResult.outcome}}">{{date createdDate}}, {{title}} (<span class="comment-count">{{properties.commentCount}}</span> - <span class="merge-outcome {{properties.mergeResult.outcome}}"></span>)</summary>
    	 <div class="item__content">
    		<h3><a href="{{links.self.0.href}}">{{title}}</a></h3>
    		<p>Created by: {{author.user.displayName}}. <span class="item__snooze"></span><br/>
            Comment count, {{properties.commentCount}}. Merge outcome, {{properties.mergeResult.outcome}}</p>
    		<pre>{{description}}</pre>
...
{{/stash}}`
            ,"PopupTemplateBBCloud":`{{#stash}}
    <details class="item item--reviewer-approved-{{ reviewerApproved }} item--reviewer-approved-count-{{approveCount}} {{#snoozed}}item--snoozed{{/snoozed}}" data-id="{{id}}">
    	 <summary>{{date createdDate}} {{title}} {{#properties.commentCount}} - <span class="item__summary-comment-count" title="Comments: {{.}}"><span class="item__count">{{.}}</span></span>{{/properties.commentCount}}</summary>
    	 <div class="item__content">
    		<h3><a href="{{links.html.href}}">{{title}}</a></h3>
    		<p>Created by: {{author.user.displayName}}. <span class="item__comment-count">Comments: <span class="item__count">{{attributes.commentCount}}</span></span> <span class="item__snooze"></span></p>
     	<pre>{{description}}</pre>
    		<div class="cell item__reviewers">
    			<h3>Reviewers:</h3> 
    			{{#reviewers}}
...
{{/stash}}`
        ...
        HELP : {
            ...
            ,"PopupIgnoreList": 'Comma separated list of PR tiles to be ignored'
            ...
            ,"BitBucketHelp": "<dl><dt>For BitBucket Server latest API use</dt><dd>/rest/api/latest/inbox/pull-requests?... your options ...</dd><dt>For legacy STASH use</dt><dd>/rest/inbox/latest/pull-requests?... your options ...</dd><dt>For BitBucket Cloud use (Private Bitbucket Cloud not yet supported)</dt><dd>/repositories/{account}/{repo}/pullrequests?q=state=\"OPEN AND\"+... your options ...</dd></dl><p>See <a href=https://developer.atlassian.com/bitbucket/api/2/reference/meta/filtering#query-pullreq target=_blank>API reference</a> and <a href=https://docs.atlassian.com/bitbucket-server/rest/6.3.2/bitbucket-rest.html#idp82  target=_blank>bitbucket rest</a></p>"
            ,"BitBucketCloudURL": "For BitBucket cloud; use https://api.bitbucket.org/2.0 <br/>NB: Currently, only public BB cloud repos are supported"
        }
