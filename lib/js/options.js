    ...
    options = {
        ops:{
            "input":[
                ...
                {"name":"API","label":"Stash JSON url","type":"text", className:"allowDblClickReset",  parent:'advanced'},
                {"name":"AuthorAPI","label":"Stash Author JSON url","type":"text", className:"allowDblClickReset",  parent:'advanced'},
                {"name":"BitBucketHelp","type":"title","tag":"p", parent:'advanced', "label":"", "className": "ml"},
                {"name":"AllowNotifications","label":"Show notifications","type":"checkbox"},
                {"name":"RefreshTime","label":"Refresh time","type":"text", className:"allowDblClickReset","html5":"range",  parent:'advanced',ops:{"min":5,"max":120,"step":5,"range-type":"s"}},
                ...
                {"name":"PopupTitleLink","label":"Title link URL","type":"text",  parent:'advanced'},
                {"name":"PopupTemplate","label":"Mustache template","type":"textarea",  parent:'advanced', className:"allowDblClickReset"},
                {"name":"PopupDateformat","label":"Date format","type":"text", parent: 'advanced'},
                ...
                {"name":"InsertCSS","label":"Insert CSS/JS into stash pull request page?","type":"checkbox",  parent:'advancedStash'},
                {"name":"StashCSS","label":"Stash pull request CSS","type":"textarea",  parent:'advancedStash', className:"allowDblClickReset"},
                {"name":"StashJS","label":"Stash pull request JS","type":"textarea",  parent:'advancedStash', className:"allowDblClickReset"}
            ]
        },
        init:function(){
        ...
        setupForm:function(){
            ...
            for (var i = 0; i<options.ops.input.length; i++){
                ...
                if (item.type === "button"){
                    ...
                    $(parent)
                        .append('<'+ tag +' id="'+ id +'">'+ (item.label || HELP[id] || "value not set") +'</'+tag+'>');
...
$(document).ready(function(){
    ...
    $('.json-viewer').on('click',function(e){
        ...
        if (backgroundPage.kmj.items && backgroundPage.kmj.items.length){
            ...
            while (item.reviewers && item.reviewers.length && item.reviewers.length > 1){
                item.reviewers.pop();// only send one reviewer json
            }
