(function($, can, chrome, window){
	...
	var backgroundPage = chrome.extension.getBackgroundPage(),
        popup = {
			...
			snoozeItem: function(e){
				...
				if (id && backgroundPage.kmj.snoozedItems.indexOf(id) === -1){
					$(e.target).closest("[data-id]").addClass("item--snoozed").attr("open",null);
    ...
    $(window).on('load', function(){
		...
		$(".display .item__snooze").each(function(i, el){
			$(el)
